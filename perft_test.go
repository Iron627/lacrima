...
const standardEPD = `rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1 ;D1 20 ;D2 400 ;D3 8902 ;D4 197281 ;D5 4865609 ;D6 119060324
...
r3k1r1/8/8/8/8/8/8/R3K2R b KQq - 0 1 ;D1 25 ;D2 547 ;D3 13579 ;D4 316214 ;D5 7848606 ;D6 190755813
...
func TestStandardPerft(t *testing.T) {
	full := os.Getenv("LACRIMA_FULL_PERFT") == "1"
	for _, tc := range parseStandardEPD(standardEPD) {
		tc := tc
		if !full && tc.depth > 3 {
			t.Run(tc.name, func(t *testing.T) {
				t.Skipf("skipping depth %d in default run; set LACRIMA_FULL_PERFT=1 for full perft", tc.depth)
			})
			continue
		}
		t.Run(tc.name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("PositionFromFEN(%q): %v", tc.fen, err)
			}
			nodes := Perft(&pos, tc.depth)
			if nodes != tc.nodes {
				t.Fatalf("perft(%d) = %d, want %d", tc.depth, nodes, tc.nodes)
			}
		})
	}
}

func parseStandardEPD(data string) []perftCase {
	var cases []perftCase
	...
	for s.Scan() {
		...
		for _, part := range parts[1:] {
			...
			cases = append(cases, perftCase{name: fen + "/d" + strconv.Itoa(depth), fen: fen, depth: depth, nodes: nodes})
		}
	}
	return cases
}
